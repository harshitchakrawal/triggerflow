...
const ReelCard = ({
  ...
  <div className="bg-white/80 border border-black/[0.07] rounded-2xl p-4 flex items-center gap-4 hover:bg-white hover:border-black/10 transition-all group backdrop-blur-sm shadow-sm">
    ...
    <div className="flex-1 min-w-0">
      ...
      <p className="text-[11px] text-[#aaa] font-medium leading-tight">
        Keyword: <span className="text-[#5c5c5c]">"{keyword}"</span> • {triggers} triggers
      </p>
    </div>
    <span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${status === "Active" ? "bg-[#f05a28]/10 text-[#f05a28]" : "bg-black/[0.05] text-[#aaa]"}`}>
      {status}
    </span>
  </div>
);
  ...
  <div className="flex items-start gap-3 py-3 border-b border-black/[0.05] last:border-0 group">
    ...
    <div className="flex-1 min-w-0">
      <p className="text-xs text-[#5c5c5c] leading-snug">
        <span className="font-bold text-[#1a1a1a]">@{user}</span> {action} • <span className="text-[#aaa] italic">{status}</span>
      </p>
    </div>
    <span className="text-[10px] text-[#aaa] font-medium whitespace-nowrap">{time}</span>
  </div>
);
    ...
    >
      <div className="relative z-10 mx-auto w-full max-w-7xl">
          ...
          <div className="flex items-center gap-4">
            <div className="bg-white/80 border border-black/[0.07] rounded-xl px-4 py-2 flex items-center gap-3 backdrop-blur-sm shadow-sm">
              <div className="text-right">
                <p className="text-[10px] text-[#aaa] font-bold uppercase tracking-widest leading-none mb-1">Connected:</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          ...
          <StatCard label="Total all time" value="234" subValue="Since Mar 1" subColor="text-[#aaa]" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-5">
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-[10px] font-bold text-[#aaa] hover:text-[#f05a28] transition-colors">See all →</button>
            </div>
